...
data = pd.read_csv( r'C:\Users\gauta\PycharmProjects\MachineLearning\Re Inforcement Learning\Topic 24 - Artificial Neural Network( ANN )\Churn_Modelling.csv' )
print( f"data.head()\n :- { data.head() }\n" )
...
print( f"After Label Encoding, x.dtypes :- \n{ x.dtypes }\n" )
...
#     No. of Input nodes for Hidden layer/ Output nodes for input layer = ????
#
# No. of Input nodes for Hidden layer/ Output nodes for input layer = ( No. of Input Layer + No. of Output Layers ) / 2
...
ann.fit( x_train, y_train, batch_size = 10, nb_epoch = 5 )

#This is not the final answer. This is the probability answer
...
plt.hist( y_test, label='y_test', rwidth=0.6 )
plt.hist( y_pred_num, label='y_pred', rwidth=0.4 )
plt.legend()
plt.show()
